...
export function MessageChat({ 
  ...
  const markMessagesAsRead = async (messagesToMark: Message[]) => {
    const unreadMessageIds = messagesToMark
      .filter(msg => !msg.is_read && msg.sender_type === 'account_user')
      .map(msg => msg.id)
  ...
  const sendMessage = async () => {
    ...
    const messageData = {
      ...
      senderType: 'client',
      clientId: null, // We'll need to get this from context
    }
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {messages.length === 0 ? (
            ...
            messages.map((message) => (
              ...
              >
                <div className={`flex items-start space-x-2 max-w-[80%] ${message.sender_type === 'client' ? 'flex-row-reverse space-x-reverse' : ''}`}>
                  <Avatar className="h-8 w-8">
                    <AvatarFallback 
                      className="text-xs"
                      style={{ 
                        backgroundColor: message.sender_type === 'client' ? brandColor : '#6B7280',
                        color: 'white'
                      }}
                    >
                  ...
                  style={{
                    backgroundColor: message.sender_type === 'client' ? brandColor : undefined,
                    color: message.sender_type === 'client' ? 'white' : undefined
                  }}>
                    <div className="text-sm">{message.content}</div>
                    <div className={`text-xs mt-1 ${
                      message.sender_type === 'client' ? 'text-white/70' : 'text-gray-500'
                    }`}>
